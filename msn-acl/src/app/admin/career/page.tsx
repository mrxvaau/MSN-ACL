...
const schema = z.object({
  title: z.string().min(1), department: z.string().optional(), location: z.string().optional(), deadline: z.string().optional(), description: z.string().optional(), applyEmail: z.string().optional(), isPublished: z.boolean().default(true)
});
...
export default function JobPostingAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/career/${editingItem.id}` : "/api/career";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/career/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">JobPostings</h1>
        </div>
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { title: "", department: "", location: "", deadline: "", description: "", applyEmail: "", isPublished: true }}
        onSubmit={handleSave}
