...
const schema = z.object({
  name: z.string().min(1), logoUrl: z.string().optional(), isPublished: z.boolean().default(true)
});
...
export default function FundingAgencyAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/funding-agencies/${editingItem.id}` : "/api/funding-agencies";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/funding-agencies/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  const handleTogglePublish = async (id: string, isPublished: boolean) => {
    ...
    try {
      const res = await fetch(`/api/funding-agencies/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleReorder = async (newItems: any[]) => {
    const res = await fetch("/api/funding-agencies/reorder", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">FundingAgencys</h1>
        </div>
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { name: "", logoUrl: "", isPublished: true }}
        onSubmit={handleSave}
