...
const schema = z.object({
  title: z.string().min(1), slug: z.string().min(1), excerpt: z.string().optional(), content: z.string().optional(), coverImage: z.string().optional(), publishedAt: z.string().optional(), isPublished: z.boolean().default(true)
});
...
export default function NewsPostAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/news/${editingItem.id}` : "/api/news";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/news/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">NewsPosts</h1>
        </div>
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { title: "", slug: "", excerpt: "", content: "", coverImage: "", publishedAt: new Date().toISOString().split('T')[0], isPublished: true }}
        onSubmit={handleSave}
