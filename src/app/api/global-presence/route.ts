...
export async function GET() {
  try {
    const items = await prisma.globalPresence.findMany({
      orderBy: { order: "asc" },
    });
...
export async function POST(req: NextRequest) {
  try {
    ...
    let order = 0;
    try {
      const lastItem = await prisma.globalPresence.findFirst({ orderBy: { order: "desc" } });
    ...
    const newItem = await prisma.globalPresence.create({
      data: { ...data, order },
    });
