...
export async function GET() {
  try {
    const items = await prisma.fundingAgency.findMany({
      orderBy: { order: "asc" },
    });
...
export async function POST(req: NextRequest) {
  try {
    ...
    let order = 0;
    try {
      const lastItem = await prisma.fundingAgency.findFirst({ orderBy: { order: "desc" } });
    ...
    const newItem = await prisma.fundingAgency.create({
      data: { ...data, order },
    });
