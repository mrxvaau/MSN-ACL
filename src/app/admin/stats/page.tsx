...
const schema = z.object({
  label: z.string().min(1), value: z.number(), suffix: z.string().optional()
});
...
export default function StatAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/stats/${editingItem.id}` : "/api/stats";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/stats/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { label: "", value: 0, suffix: "" }}
        onSubmit={handleSave}
