...
const schema = z.object({
  title: z.string().min(1), fileUrl: z.string().optional()
});
...
export default function PolicyAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/policies/${editingItem.id}` : "/api/policies";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/policies/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Policys</h1>
        </div>
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { title: "", fileUrl: "" }}
        onSubmit={handleSave}
