...
const schema = z.object({
  title: z.string().min(1, "Title is required"), subtitle: z.string().optional(), imageUrl: z.string().optional(), ctaText: z.string().optional(), ctaLink: z.string().optional(), isPublished: z.boolean().default(true)
});
...
export default function HeroSlideAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/hero-slides/${editingItem.id}` : "/api/hero-slides";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/hero-slides/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">HeroSlides</h1>
        </div>
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { title: "", subtitle: "", imageUrl: "", ctaText: "", ctaLink: "", isPublished: true }}
        onSubmit={handleSave}
