...
const schema = z.object({
  name: z.string().min(1), logoUrl: z.string().optional(), websiteUrl: z.string().optional(), isPublished: z.boolean().default(true)
});
...
export default function ClientAdminPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/clients/${editingItem.id}` : "/api/clients";
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/clients/${itemToDelete.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <CrudFormDrawer
        ...
        schema={schema}
        fields={formFields}
        defaultValues={editingItem || { name: "", logoUrl: "", websiteUrl: "", isPublished: true }}
        onSubmit={handleSave}
